...
import type { GameState, PaddleUpdate, BallUpdate, StateUpdate } from './types.js';
import {
  createGameState,
  updatePaddle,
  gameTick,
...
let games: ReadonlyMap<string, GameState> = new Map();

// Pure function: Get or create game state
const getOrCreateGame = (gameId: string): GameState => {
...
const setGame = (gameId: string, state: GameState): void => {
...
client.on('message', (topic, message) => {
  try {
    ...
    if (parts[0] !== 'pong' || parts[1] !== 'game') return;
    ...
    if (messageType === 'p1' || messageType === 'p2') {
      const paddleData = data as PaddleUpdate;
    ...
    if (messageType === 'join') {
      const joinData = data as { player: 1 | 2; timestamp: number };
    ...
    if (messageType === 'serve' && state.status === 'playing' && state.servingPlayer !== null) {
      const serveData = data as { player: 1 | 2; timestamp: number };
...
function publishPaddle(gameId: string, player: 1 | 2, y: number): void {
  const update: PaddleUpdate = {
    y,
    timestamp: Date.now(),
  };
...
function publishBall(gameId: string, ball: GameState['ball']): void {
  const update: BallUpdate = {
    x: ball.x,
    y: ball.y,
    dx: ball.dx,
    dy: ball.dy,
...
function publishGameState(gameId: string, state: GameState): void {
